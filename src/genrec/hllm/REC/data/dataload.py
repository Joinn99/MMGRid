...
class Data:
    ...
    def build(self):
        ...
        if self.config['train_timestamp_start']:
            ...
            # train_inter_feat = self.inter_feat[self.inter_feat["timestamp"] < self.config['train_timestamp_start']]
            # user_last_timestamp = train_inter_feat.groupby('user_id')["timestamp"].agg("max")
            # valid_users = user_last_timestamp[user_last_timestamp >= self.config['train_timestamp_split']].index
            # self.logger.info(f"Valid users: {len(valid_users)}")
            # self.inter_feat = self.inter_feat[self.inter_feat['user_id'].isin(valid_users)]
            # self.logger.info(f"Train interactions after filtering: {len(self.inter_feat)}")
